...
class DigitalTwinTimeSeries():
    def __init__(self, path: str, to_iso3: bool = True) -> None:
        """
    ...
    def _preprocess(self, path: str) -> pd.DataFrame:
        ...
        data = data[data.columns[-n_meta_columns:].tolist() + data.columns[:-n_meta_columns].tolist()]
        
        # Clean numerical values
        numerical_columns_i = n_meta_columns
        data.iloc[:,numerical_columns_i:] = data.iloc[:,numerical_columns_i:].replace("[a-zA-Z: ]", "", regex=True).replace("", 0, regex=True).astype(np.float32)
    ...
    def melt_data(self, category_column: str) -> Dict[str,pd.DataFrame]:
        ...
        for category in categories:
            ...
            data_slice_melted = data_slice.melt(id_vars="geo", value_vars=self.data.columns[first_year_i:], var_name="year")
